class Scores{
  ...
  void showScores(){
    ...
    text("PUNTAJE:"+nf(score,10), width/2, height-35);
    ...
    if(player.health > 0){
      ...
      fill(#ed2525);
      rect(width/2-400+mappedValue/2,height-32,mappedValuer,25);
    }
    fill(#ffffff);
    text("SALUD:", width/2-465, height-25);
    ...
    if(boss != null && boss.active){
      ...
      fill(#ed2525);
      rect(width/2-400+mappedValue/2,height-32,mappedValuer,25);
      text("JEFE",width/2+465, height-25);
    }
    ...
    if (shotHit == 0)
      ...
      text("HIT/SHOT:NA"+(int)(((shotHit*1.0)/(shot*1.0))*100)+"%",width/2, height-10);
    ...
    if(!gameOver && streak > 2){
      ...
      text("RACHA DE"+streak,width/2,height-50);
    }
  }
